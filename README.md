...
### Quantumult X
導入 [naruto-fe.snippet](.qx/naruto-fe.snippet)（QX → 設定 → 下載 → 從 URL 下載配置）
或手動加入 rewrite + mitm 規則。

### Surge
導入 [naruto-fe.sgmodule](.surge/naruto-fe.sgmodule)（Surge → 模組 → 安裝新模組）

### Loon
導入 [naruto-fe.plugin](.loon/naruto-fe.plugin)（Loon → 插件 → 從 URL 安裝）
